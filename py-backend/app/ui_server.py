...
class UiRequestHandler(BaseHTTPRequestHandler):
    ...
    def _handle_draft_sample(self, payload: dict[str, Any]) -> dict[str, Any]:
        return handle_draft_sample(self.server.settings, payload)

    def _handle_run_extraction(self, payload: dict[str, Any]) -> dict[str, Any]:
        return handle_run_extraction(self.server.settings, payload)
...
# These take (settings, payload) and return a JSON-serializable dict, shared by
# both the stdlib http.server handler (local) and the FastAPI app (Vibe hosted).
# They raise ValueError for client errors (mapped to HTTP 400 by each transport).


def handle_draft_sample(settings: Settings, payload: dict[str, Any]) -> dict[str, Any]:
    # Instructions are OPTIONAL — the schema/sample-drafting guide defines column
    # behavior. A source document or URL is still required to draft against.
    ...
    if not document_files and not source_urls:
        raise ValueError("Add at least one source document or URL to generate a draft sample CSV.")
...
def handle_run_extraction(settings: Settings, payload: dict[str, Any]) -> dict[str, Any]:
    instructions = str(payload.get("instructions", "")).strip()
    program_filter = str(payload.get("program_filter", "")).strip() or None
    source_urls = _parse_source_urls(payload.get("source_urls"))
    ...
    batch_result = run_batches(request, settings)[0]
    _persist_batch_deliverables(batch_name)
    return {
        "result": serialize_batch_result(batch_result),
        "batch": load_batch_detail(batch_name),
    }
...
def _persist_batch_deliverables(batch_name: str) -> None:
    """Mirror a batch's final + clean extracted CSV to S3. No-op unless S3 enabled."""
    if not s3_persistence.is_enabled():
        return
    output_dir = get_batch_root(batch_name) / "output"
    for csv_path in (find_output_csv(output_dir), find_clean_output_csv(output_dir)):
        if csv_path is not None:
            s3_persistence.upload_file(batch_name, csv_path)


def handle_audit_batch(settings: Settings, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    result = audit_extracted_csv(output_csv, batch_settings, runtime_paths=runtime_paths)
    return {
        "audit": {
...
def _derive_requested_sample_row_target(instructions: str) -> int | None:
    ...
    if range_match:
        ...
        return max(MIN_SAMPLE_ROWS, min(MAX_SAMPLE_ROWS, requested))
...
def persist_sample_csv(batch_name: str, filename: str, content: str) -> Path:
    ...
    # Persist the approved sample to S3 under a canonical name so a redeployed pod
    # (empty disk) can recover it. No-op unless S3 is configured.
    s3_persistence.upload_file(batch_name, path, remote_name="approved_sample.csv")
    return path
...
def find_existing_sample_csv(batch_name: str) -> Path | None:
    ...
    # Not on local disk — try S3 (redeployed pod). Restores under the canonical name.
    restored = batch_root / "approved_sample.csv"
    if s3_persistence.download_file(batch_name, "approved_sample.csv", restored):
        return restored
    return None
...
def list_batch_summaries() -> list[dict[str, Any]]:
    # Local batches first (fast, full metadata), then any batch that exists only in
    # S3 (e.g. after a redeploy wiped local disk) so history survives.
    local_names: list[str] = []
    if CHAT_BATCH_OUTPUT_DIR.exists():
        local_names = [
            batch_root.name
    ...
    for name in local_names:
        seen.add(name)
        batch_root = get_batch_root(name)
        output_dir = batch_root / "output"
        output_csv = find_output_csv(output_dir)
        summaries.append(
            {
                ...
                "document_count": len(list_existing_document_files(name)),
    ...
    # S3-only batches (no local dir): surface them as recoverable deliverables.
    for name in s3_persistence.list_batch_names():
        if name in seen:
            continue
        summaries.append(
            {
...
def find_output_csv(output_dir: Path) -> Path | None:
    ...
    if output_dir.exists():
        ...
        if candidates:
            return candidates[0]
    # Local miss — try S3 (redeployed pod). The final CSV is named <batch_name>.csv.
    return _restore_from_s3(output_dir, f"{_batch_name_for(output_dir)}.csv")
...
def find_clean_output_csv(output_dir: Path) -> Path | None:
    ...
    if output_dir.exists():
        ...
        if candidates:
            return candidates[0]
    return _restore_from_s3(output_dir, f"{_batch_name_for(output_dir)}.clean.csv")


def _batch_name_for(output_dir: Path) -> str:
    """Batch name from an output dir (…/chat_batches/<batch_name>/output)."""
    return output_dir.parent.name


def _restore_from_s3(output_dir: Path, filename: str) -> Path | None:
    """Download a deliverable from S3 into output_dir. Returns the path or None.

    No-op (None) unless S3 is enabled. Lets a redeployed pod serve past batches.
    """
    if not s3_persistence.is_enabled() or not filename or filename.startswith("."):
        return None
    batch_name = _batch_name_for(output_dir)
    target = output_dir / filename
    if s3_persistence.download_file(batch_name, filename, target):
        return target
    return None
