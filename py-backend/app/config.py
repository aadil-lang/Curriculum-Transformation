...
class Settings:
    extractor_model: str = field(default_factory=lambda: os.getenv("EXTRACTOR_MODEL", "gemini-3.0-flash-preview"))
    extractor_provider: str = field(default_factory=lambda: os.getenv("EXTRACTOR_PROVIDER", "gemini"))
    # Sample-draft model: fast/cheap tier used only for the --draft-only preview path.
    # Falls back to the main extractor model/provider when unset.
    draft_extractor_model: str = field(
        default_factory=lambda: os.getenv("DRAFT_EXTRACTOR_MODEL", "").strip()
    )
    draft_extractor_provider: str = field(
        default_factory=lambda: os.getenv("DRAFT_EXTRACTOR_PROVIDER", "").strip()
    )
    ...
    extractor_max_tokens: int = field(
        default_factory=lambda: max(4000, int(os.getenv("EXTRACTOR_MAX_TOKENS", "65000")))
    )
    extraction_citations_enabled: bool = field(
        default_factory=lambda: os.getenv("EXTRACTION_CITATIONS", "0").strip().lower() in {"1", "true", "yes", "on"}
    )
    pdf_ocr_mode: Literal["off", "auto", "always"] = field(
        default_factory=lambda: (
            mode
            if (mode := os.getenv("PDF_OCR_MODE", "auto").strip().lower()) in {"off", "auto", "always"}
            else "auto"
        )
    )
    pdf_ocr_min_chars_per_page: int = field(
        default_factory=lambda: max(20, int(os.getenv("PDF_OCR_MIN_CHARS_PER_PAGE", "80")))
    )
    pdf_ocr_low_text_page_ratio: float = field(
        default_factory=lambda: min(1.0, max(0.0, float(os.getenv("PDF_OCR_LOW_TEXT_PAGE_RATIO", "0.5"))))
    )
    pdf_ocr_language: str = field(
        default_factory=lambda: os.getenv("PDF_OCR_LANGUAGE", "eng+fra").strip() or "eng+fra"
    )
    pdf_ocr_dpi: int = field(
        default_factory=lambda: max(72, int(os.getenv("PDF_OCR_DPI", "300")))
    )
    progression_matrix_option_tracks: tuple[str, ...] = field(
        default_factory=lambda: tuple(
            track.strip()
            for track in os.getenv("PROGRESSION_MATRIX_OPTION_TRACKS", "CST,TS,S").split(",")
            if track.strip()
        )
    )
