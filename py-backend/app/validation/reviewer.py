        ...
        if contract and contract.always_empty_columns:
            for output_column in contract.always_empty_columns:
                ...
                if not internal_name:
                    continue
                has_citation = (
                    self.settings.extraction_citations_enabled
                    and (data.get(f"{internal_name}_source_citation") or "") != ""
                )
                if (data.get(internal_name) or "") != "" or has_citation:
                    cleared_value = data.get(internal_name)
                    data[internal_name] = None
                    if self.settings.extraction_citations_enabled:
