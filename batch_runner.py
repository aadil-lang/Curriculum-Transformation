...
# Columns forced blank in a sample draft. l3 is NOT here: it may be filled when the source
# genuinely has a third hierarchy level (the sample contract's field_placement_rules allow it).
# l4/l5/Standard code/czi stay blank by default in sample mode.
DRAFT_ALWAYS_BLANK_COLUMNS = {"l4", "l5", "standard code", "czi_standard_code"}
...
def create_model_settings(settings: Settings, schema_path: Path) -> Settings:
    return Settings(
        ...
        draft_extractor_model=settings.draft_extractor_model,
        draft_extractor_provider=settings.draft_extractor_provider,
...
def _run_sample_extraction_draft(
    *,
    ...
    """Fast sample draft: one region-targeted, single-chunk extraction per input via
    ``ExtractionEngine.draft_sample_rows`` — no chunk fan-out, no review/critic fix-loop.

    The draft rows are then normalized and audited by the same deterministic (non-LLM)
    helpers as before, so the valid/invalid contract for the UI is unchanged.
    """
    ...
    draft_settings = batch_settings
    if settings.draft_extractor_model:
        draft_settings = replace(
            batch_settings,
            extractor_model=settings.draft_extractor_model,
            portkey_extractor_provider=(
                settings.draft_extractor_provider or batch_settings.portkey_extractor_provider
            ),
        )
    engine = ExtractionEngine(draft_settings)
    source_url_map = source_url_map or {}

    sample_row_target = _normalize_sample_row_target(batch.sample_row_target)
    process_results: list[dict[str, str]] = []
    headers = _sample_template_headers(schema_config)
    sample_output_path = runtime_paths.output_dir / "sample_output_template.csv"

    # Replace scaffold/example rows immediately so the preview only ever shows
    # real drafted rows or an intentionally empty draft.
    _write_sample_rows_to_csv(sample_output_path, headers, [])

    drafted_rows: list[dict[str, str]] = []
    for path in resolved_input_files:
        try:
            parsed_document = parse_input(path)
            payload_rows = engine.draft_sample_rows(
                parsed_document, max_rows=sample_row_target, min_rows=min(MIN_SAMPLE_ROWS, sample_row_target)
            )
            canonical_source = _draft_canonical_source(parsed_document, source_url_map)
            drafted_rows.extend(
                _draft_payload_to_header_dict(row, schema_config, headers, canonical_source)
                for row in payload_rows
            )
            process_results.append(
                {"path": str(path), "status": "drafted", "message": f"Drafted {len(payload_rows)} row(s).", "stage": "sample_draft"}
            )
        except Exception as exc:  # noqa: BLE001 - a draft failure is per-file, non-fatal
            LOGGER.exception("Sample draft failed for %s:", path)
            process_results.append(
                {"path": str(path), "status": "manual_review", "message": str(exc), "stage": "sample_draft"}
            )
        if len(drafted_rows) >= sample_row_target:
            break

    selected_rows = _select_sample_rows(drafted_rows, schema_config, sample_row_target)
    ...
    if review_payload["status"] == "valid":
        _write_sample_rows_to_csv(sample_output_path, headers, sample_rows)
    else:
        # Avoid leaving misleading placeholder rows behind when the draft is incomplete.
        _write_sample_rows_to_csv(sample_output_path, headers, [])
...
def _draft_canonical_source(parsed_document: Any, source_url_map: dict[str, str]) -> str:
    """Best canonical source link for drafted rows (mirrors pipeline._canonical_source)."""
    source_path = str(getattr(parsed_document, "source_path", "") or "")
    if source_path in source_url_map:
        return source_url_map[source_path]
    metadata = getattr(parsed_document, "metadata", {}) or {}
    url = str(metadata.get("url") or "").strip()
    return url or source_path


def _draft_payload_to_header_dict(
    payload_row: BaseModel,
    schema_config: TargetSchemaConfig,
    headers: list[str],
    canonical_source: str,
) -> dict[str, str]:
    """Flatten one extraction payload model into a sample-template header-keyed dict."""
    data = payload_row.model_dump()
    by_output_column: dict[str, str] = {}
    for spec in schema_config.fields:
        output_column = spec.output_column or spec.name
        value = data.get(spec.name)
        by_output_column[output_column] = "" if value is None else str(value)
    # Stamp source when the model left it blank (canonical URL is rarely verbatim in-body).
    source_header = _find_header(headers, "source")
    if source_header and not (by_output_column.get(source_header, "") or "").strip():
        by_output_column[source_header] = canonical_source
    return {header: by_output_column.get(header, "") for header in headers}


def _normalize_sample_row_target(raw_value: int | None) -> int:
    # With no explicit request, aim for the ceiling so the draft has room to include
    # one row per source variation (sections, code formats, option tracks, shapes).
    # The floor still guarantees at least MIN_SAMPLE_ROWS.
...
def _select_sample_rows(
    ...
    """Cap drafted rows to the target while PRESERVING variety.

    The draft is already curated to showcase the source's structural variations
    (sections, code formats, option tracks, row shapes), so we keep the model's order
    and simply trim to the target rather than collapsing to a single consecutive
    same-source block (which would discard rows from other sections/documents).
    """
    if not rows:
        return []
...
# Program/course labels that name a course, not a learner grade band. These must never
# land in the grade columns even if the model puts them there.
_PROGRAM_GRADE_LABELS = {
    "ap",
    "advanced placement",
    "ib",
...
# grade_level buckets are valid for grade_level itself but not for display_grade/grade_number.
_GRADE_LEVEL_BUCKETS = {"elementary school", "middle school", "high school"}


def _sanitize_grade_field(value: str, *, expand_ranges: bool) -> str:
    """Strip program/course labels and grade buckets from a grade cell.

    ``display_grade`` and ``grade_number`` must hold an actual learner band (a grade,
    year, stage, or range), never a program name like 'AP' or a bucket like
    'High School'. A bad value is blanked so the draft audit surfaces it rather than
    the pipeline shipping a wrong grade. When ``expand_ranges`` is set (grade_number),
    a range like '9-12' is expanded to '9,10,11,12'.
    """
    text = value.strip()
    if not text:
        return ""
    lowered = text.lower()
    if lowered in _PROGRAM_GRADE_LABELS or lowered in _GRADE_LEVEL_BUCKETS:
        return ""
    if expand_ranges:
        range_match = re.fullmatch(r"\s*(\d{1,2})\s*-\s*(\d{1,2})\s*", text)
        if range_match:
...
def _normalize_sample_rows_for_draft(
    ...
    for row in rows:
        ...
        for header in headers:
            ...
            if lowered_header in DRAFT_ALWAYS_BLANK_COLUMNS:
                normalized_row[header] = ""
                continue
            if lowered_header == "source":
                normalized_row[header] = _sanitize_sample_source_value(value)
                continue
            if lowered_header == "display_grade":
                normalized_row[header] = _sanitize_grade_field(value, expand_ranges=False)
                continue
            if lowered_header == "grade_number":
                normalized_row[header] = _sanitize_grade_field(value, expand_ranges=True)
                continue
            normalized_row[header] = value
        # domain must never be empty; topic may be. If the model left domain blank but
        # filled topic, promote topic up into domain (never the reverse) so every row has
        # a grouping level. Deterministic guarantee behind the prompt rule.
        domain_header = _find_header(headers, "domain")
        topic_header = _find_header(headers, "topic")
        if domain_header and topic_header:
            domain_val = (normalized_row.get(domain_header, "") or "").strip()
            topic_val = (normalized_row.get(topic_header, "") or "").strip()
            if not domain_val and topic_val:
                normalized_row[domain_header] = topic_val
                normalized_row[topic_header] = ""
